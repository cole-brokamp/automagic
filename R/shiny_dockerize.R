...
shiny_dockerize <- function(directory=getwd(),re.automagic=FALSE,
  ...
  if (!file.exists(file.path(directory,'.dependencies')) | re.automagic) {
    if (re.automagic) unlink(file.path(directory,'.dependencies'))
  ...
  # if no shiny server config file, make one
  if (!file.exists(file.path(directory,'shiny-server.conf'))) {
    file.copy('/Users/cole/Documents/Biostatistics/_CB/shiny_docker/shiny-server.conf',
              file.path(directory,'shiny-server.conf'))
  }
